$(document).ready(function(){
    ...
    $('#client_sync_btn').off('click').on('click', function(target){
        ...
        $.ajax({
            ...
            beforeSend: function (xhr) {
                $('#client_sync_result_div').html("<i class='fa fa-spinner fa-spin fa-3x fa-fw'></i>");
                $("#client_total_sync_duration").html("");
            },
            async: true
        }).done(function (response) {
           ...
           result = JSON.parse(response);
            if( result ){
                ...
                result.forEach(element => {
                    ...
                    $("#client_total_sync_duration").html(totalDueration);
                    /*str = str + "<li>url: " + element.url + "</li>" +
                    "<li>len: " + element.charLen + "</li>" + 
                    "<li>duration: " + element.duration + "</li>"; */
                    
                    $('#client_sync_result_div').html(str);
                });
            }
            console.log(response);
            ...
            console.log(xhr);
            $('#client_sync_result_div').html("Sorry error check console log");
            $("#client_total_sync_duration").html("");
        });
    ...
    $('#client_async_btn').off('click').on('click', function(target){
        $.ajax({
            type: "get",
            url: "http://shirak-dell/CFSummit2018/work/UrlHelper.cfc?method=startWebSiteDownloadASync",
            
            beforeSend: function (xhr) {
                $('#client_async_result_div').html("<i class='fa fa-spinner fa-spin fa-3x fa-fw'></i>");
                $("#client_total_async_duration").html("");
            },
            async: true
        }).done(function (response) {
            result = JSON.parse(response);
            if( result ){
                ...
                result.forEach(element => {
                    ...
                    $("#client_total_async_duration").html(totalDueration);
                    /*    str = str + "<li>url: " + element.url + "</li>" +
                    "<li>len: " + element.charLen + "</li>" + 
                    "<li>duration: " + element.duration + "</li>"; 
                    */
                    $('#client_async_result_div').html(str);
                });
            }
            ...
            console.log(xhr);
            $('#client_async_result_div').html(statusText);
            $("#client_total_async_duration").html("");
        });
        
    });

    $('#client_async_await_btn').off('click').on('click', function(target){
        $.ajax({
            type: "get",
            url: "http://shirak-dell/CFSummit2018/work/UrlHelper.cfc?method=startWebSiteDownloadASyncAwait",
            ...
            beforeSend: function (xhr) {
                $('#client_async_result_div').html("<i class='fa fa-spinner fa-spin fa-3x fa-fw'></i>");
                $("#client_total_async_await_duration").html("");
            },
            async: true
        }).done(function (response) {
            result = JSON.parse(response);
            if( result ){
                ...
                result.forEach(element => {
                    ...
                    $("#client_total_async_await_duration").html(totalDueration);
                    /*    str = str + "<li>url: " + element.url + "</li>" +
                    "<li>len: " + element.charLen + "</li>" + 
                    "<li>duration: " + element.duration + "</li>"; 
                    */
